...
class Executor:
    ...
    async def push(self, localFolder, s3Folder):
        ...
        for localPath, filename in outputPairs.items():
            tasks.append(self.api.uploadMultipart(localPath, os.path.join(s3Folder, filename)))
        await asyncio.gather(*tasks)

        for folder in folders:
            await self.push(os.path.join(localFolder, folder), os.path.join(s3Folder, folder))
